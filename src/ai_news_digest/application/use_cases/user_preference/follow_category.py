...
class FollowCategoryUseCase:
    ...
    async def execute(self, current_user: User, category_id: UUID) -> None:
        await self._preference_repository.add_followed_category(current_user.id, category_id)
...
class UnfollowCategoryUseCase:
    ...
    async def execute(self, current_user: User, category_id: UUID) -> None:
        await self._preference_repository.remove_followed_category(current_user.id, category_id)
