...
async def _detect_trends_impl() -> dict[str, int | str | float]:
    """
    Typed implementation function for trend detection.
    """
    ...
    async for container in get_container():
        ...
        detect_use_case = container.detect_trends
        if detect_use_case is None:
            logger.info("Trend detection use case is not available")
...
@celery_app.task(
    name="workers.tasks.trend.detect_trends",
    max_retries=2,
    default_retry_delay=120,
)
def detect_trends() -> dict[str, int | str | float]:
    """
    Detect trends and emerging story intelligence.
